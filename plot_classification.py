...
from sklearn import preprocessing, decomposition, svm, model_selection #, cross_validation
	...
	printFeatureImportance : bool (default False)
        ...
        if outputFormat=='score':
            ...
            for ix,clf in enumerate([clfs[x] for x in models]):
                kf = model_selection.KFold(len(self.dataset), nFolds, shuffle=True)
                scores = model_selection.cross_val_score(clf, self.dataset, self.labels, cv=kf)
        ...
        # Return a summary table describing several metrics or a confusion matrix 
            ...
            for ix,clf in enumerate([clfs[x] for x in models]):
                ...
                kf = model_selection.StratifiedKFold(self.labels, n_folds=nFolds)
        ...
        # Generate Precision-Recall curves, topK precision, or list of topK at risk scores
            ...
            for ix,clf in enumerate([clfs[x] for x in models]):
                ...
                kf = model_selection.StratifiedKFold(self.labels, n_splits=nFolds, shuffle=True)
